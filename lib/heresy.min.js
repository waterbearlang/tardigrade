var heresy = (function (document,exports) {
  ...
  var augmented = function augmented(Class) {
    ...
    var eventHandlers = prototype._events || Object.defineProperty(prototype, "_events", {
      value: Object.getOwnPropertyNames(prototype).filter(function (type) {
        return /^on/.test(type);
      // }).filter(function (type) {
      //   return !['onconnected', 'ondisconnected', 'onattributechange', 'oninit'].includes(type);
      }).map(function (type) {
        return type.slice(2);
      })
    })._events;
    console.log('eventHandlers: %o', eventHandlers);
